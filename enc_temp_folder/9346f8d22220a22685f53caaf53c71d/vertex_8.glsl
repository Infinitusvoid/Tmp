...
float f_1(float x, float y, float factor)
{
    float factor_scale = 1.0 / factor;
    return abs(sin(sin(x * factor) / cos(y * factor))) * factor_scale;
}

float f_2(float x, float y, float factor)
{
    float factor_scale = 1.0 / factor;
    return abs(sin(sin(x * factor) * cos(y * factor))) * factor_scale;
}

float f_3(float x, float y, float factor)
{
    float factor_scale = 1.0 / factor;
    return abs(sin(sin(x * factor) - cos(y * factor))) * factor_scale;
}

float f_4(float x, float y, float factor)
{
    float factor_scale = 1.0 / factor;
    return sin(sin(x) * cos(y)) * pow(factor_scale, 4.0);
}

float f_0(float x, float y, float factor)
{
    float factor_scale = 1.0 / factor;
    return sin(sin(x) / cos(y)) - sin(sin(x + factor) / cos(y + factor)) * factor_scale * 4.0;
}
...
void main()
{
    ...
    vec3 color_factor = vec3(0.0, 0.0, 0.0);
    ...
    // py += 0.2 * sin(uTime * 10000.3 + rnd_y * 120.2 + py * 100.0);
    // py += 0.2 * sin(uTime * 10000.2 + rnd_x * 140.2 + rnd_y * 100.0);
    // pz *= 4.0;
    ...
    color_r = color_factor.r;// 0.4 + 0.2 * fract(id * 0.011 + rnd_x);
    color_g = color_factor.g;// 0.4 + 0.2 * fract(id * 0.042 + rnd_x);
    color_b = color_factor.b;// 0.4 + 0.2 * fract(id * 0.024 + rnd_x);
