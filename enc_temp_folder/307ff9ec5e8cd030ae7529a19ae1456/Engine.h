...
struct EngineFileOutputFilepats
{
	void init_as_f_drive()
	{
		...
		folder_output = "F:/output/tmp/" + NameGenerators_::generate_prefix_timestamp_suffix_name("universe_", std::string("_") + std::to_string(Random::random_int(0, 99999999)) + std::string("_"));
...
template<class Fn>
void add_shader(Scene_::Scene& scene, int index, Fn&& fn) {
	Program::Shader sh(scene, index);
	std::invoke(std::forward<Fn>(fn), sh);
}
